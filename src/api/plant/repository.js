...
exports.addPlant = async (
  ...
  const query = `INSERT INTO plants (name, description, watering_interval, group_id, last_watered_at, photo_url) VALUES (?, ?, ?, ?, ?, ?)`;
...
exports.deletePlant = async (id) => {
  ...
  try {
    ...
    const photoUrl =
      resultGetPhotoUrl.length > 0 ? resultGetPhotoUrl[0].photo_url : null;
