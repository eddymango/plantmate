...
exports.createGroup = async (req, res) => {
  ...
  try {
      const groupId = await repository.createGroup(name, description, password, userId);
...
exports.getAllGroups = async (req, res) => {
  try {
      ...
      res.status(200).json({ result: 'ok', data: groups });
  } catch (err) {
...
exports.getGroupById = async (req, res) => {
  ...
  try {
      ...
      if (!group) {
          return res.status(404).json({ result: 'fail', message: '그룹을 찾을 수 없습니다.' });
      }
...
exports.joinGroup = async (req, res) => {
  ...
  try {
      ...
      if (!isValid) {
          return res.status(401).json({ result: 'fail', message: '비밀번호가 올바르지 않습니다.' });
      }
...
exports.leaveGroup = async (req, res) => {
  ...
  try {
      ...
      if (result.affectedRows === 0) {
          return res.status(404).json({ result: 'fail', message: '그룹을 찾을 수 없거나, 이미 탈퇴한 상태입니다.' });
      }
