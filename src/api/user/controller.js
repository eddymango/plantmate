...
exports.register = async(req, res) => {
  ...
  //사용자 등록
  const { affectedRows, insertId } = await repository.register(email, hashedPassword, name);
  if (affectedRows > 0) {
    const token = await jwt({ id: insertId, name });
    res.status(201).json({ result: "ok", access_token: token });
  } else {
    res.status(500).json({ result: "fail", message: "사용자 등록에 실패했습니다." });
  }
};
...
exports.login = async(req, res) => {
  ...
  if (!user) {
    res.status(401).json({ result: "fail", message: "이메일 혹은 비밀번호를 확인해 주세요." });
  } else {
...
exports.update = async (req, res) => {
  ...
  if (password) {
    hashedPassword = crypto.pbkdf2Sync(password, process.env.SALT_KEY, 50, 100, 'sha512').toString('base64');
  }

  //사용자 정보 수정
  const result = await repository.update(user.id, name, hashedPassword);
  if (result.affectedRows > 0) {
    const updatedUser = await repository.findById(user.id);
    res.status(200).json({ result: "ok", data: updatedUser });
  } else {
    res.status(500).json({ result: "fail", message: "사용자 정보 수정에 실패했습니다." });
  }
};
...
exports.delete = async (req, res) => {
  ...
  if (result.affectedRows > 0) {
    res.status(200).json({ result: "ok", message: "회원탈퇴가 완료되었습니다." });
  } else {
    res.status(500).json({ result: "fail", message: "회원탈퇴에 실패했습니다." });
  }
};
